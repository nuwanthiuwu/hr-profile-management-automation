...
test.describe('Login Module - Smart HR Admin', () => {
  ...
  test.describe('TC_LOGIN_04 - Remember Me Functionality', () => {
    test('TC_LOGIN_04.1 - Should keep user logged in after closing and reopening browser', async ({ context, page }) => {
      ...
      // App stores auth in localStorage (not cookies) — verify token persists
      const authToken = await page.evaluate(() => localStorage.getItem('smart_hr_access_token'));
      expect(authToken).toBeTruthy();
      console.log(`✅ Logged in with Remember Me. Auth token in localStorage: ${!!authToken}`);

      // Open a new tab in the same context — localStorage is shared across tabs
      const newPage = await context.newPage();
      await newPage.goto('https://smart-hr-fe.vercel.app/dashboard');
      await newPage.waitForLoadState('networkidle');

      const urlAfterReopen = await newPage.url();
      expect(urlAfterReopen).not.toContain('/login');

      await newPage.close();

      console.log('✅ Session persisted: new tab can access dashboard without re-login');
    });
  });
